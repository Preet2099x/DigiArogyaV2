                ...
                <div>
                  <ul className="space-y-3">
                    {attachments.map((attachment) => {
                      ...
                      const type = isImage ? 'image' : isPdf ? 'pdf' : 'doc';
                      
                      return (
                        <li key={attachment.id} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                          <div className="flex items-center gap-3 overflow-hidden">
                            ...
                            <div className="min-w-0 flex flex-col">
                              <span 
                                className="text-sm font-medium text-gray-700 truncate cursor-pointer hover:text-emerald-600 hover:underline"
                                title={attachment.fileName}
                                onClick={() => isImage ? handleViewImage(attachment) : handleDownload(attachment)}
                              >
                                {attachment.fileName}
                              </span>
                              <span className="text-xs text-gray-400">{attachment.fileSize ? formatFileSize(attachment.fileSize) : 'Unknown size'}</span>
                            </div>
                          </div>
